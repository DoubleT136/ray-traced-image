...
float to_rad(float deg) { return deg * PI / 180.0; }

void SceneObject::SetBlenderCoordinates(const glm::vec3& position, const glm::vec3& orientation)
{
	ResetPositionAndOrientation();
	Translate(glm::vec3(position.y, position.z, position.x));
	Rotate(glm::vec3(1.0, 0.0, 0.0), to_rad(orientation.x - 90.0));
	Rotate(glm::vec3(0.0, 1.0, 0.0), to_rad(orientation.z - 90.0));
	Rotate(glm::vec3(0.0, 0.0, 1.0), to_rad(orientation.y));
}

void SceneObject::ResetPositionAndOrientation()
{
	position = glm::vec4();
	rotation = glm::quat();
}
